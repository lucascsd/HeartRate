...
bool_t max30102_Init(max30102_t driver_config)
{
	...
	if (!max30102_readPartID(_max30102))
		return FALSE;

	/* Lectura de Revision ID */
	if (!max30102_readRevisionID(_max30102))
		return FALSE;

	/* POR register */
	if (!max30102_reset( _max30102 ))
		return FALSE;
...
{
	uint8_t data = readPartID(driver_config);
}

uint8_t readPartID(max30102_t driver_config)
{
	uint8_t data;
	driver_config._i2cReadFn(I2C0, MAX_ADDRESS, PART_ID, &data, 1);
...
{
	uint8_t data = readRevisionID(driver_config);
}

uint8_t readRevisionID(max30102_t driver_config)
{
	uint8_t data;
	driver_config._i2cReadFn(I2C0, MAX_ADDRESS, REVISION_ID, &data, 1);
...
{
	/* Sample average */
	if (!max30102_config(_max30102, FIFO_CONFIG, SMP_AVE_8, 0))
		return FALSE;

	/* Roll Over */
	if (!max30102_config(_max30102, FIFO_CONFIG, FIFO_ROLLOVER_EN, 0))
		return FALSE;

	/* Set LED Mode */
	if (!max30102_config(_max30102, MODE_CONFIG, MODE_MULTI, 0))
		return FALSE;

	/* Set ADC Range Mode */
	if (!max30102_config(_max30102, SPO2_CONFIG, SPO2_ADC_RGE_2048, 0))
		return FALSE;

	/* Set Sample Rate Mode */
	if (!max30102_config(_max30102, SPO2_CONFIG, SAMPLERATE_1000, 0))
		return FALSE;

	/* Set Pulse width Mode */
	if (!max30102_config(_max30102, SPO2_CONFIG, PULSEWIDTH_411, 0))
		return FALSE;

	/* Set Led_1 Mode */
	if (!max30102_config(_max30102, LED_PULSE_AMP_1, PULSEAMPLITUDE_12, 0))
		return FALSE;

	/* Set Led_2 Mode */
	if (!max30102_config(_max30102, LED_PULSE_AMP_2, PULSEAMPLITUDE_12, 0))
		return FALSE;

	/* Set Led_1 Config */
	if (!max30102_config(_max30102, LED_CTRL_REG_12, MULTILED_CONTROL_RED, 0))
		return FALSE;

	/* Set Led_2 Config */
	if (!max30102_config(_max30102, LED_CTRL_REG_12, MULTILED_CONTROL_IR, 4))
		return FALSE;
...
bool_t max30102_config( max30102_t driver_config, uint8_t _register, uint8_t _param, uint8_t shitf)
{
	...
	driver_config._i2cReadFn(I2C0, MAX_ADDRESS, _register, &_config._buffer, 1);
	datoActual = _config._buffer;

	/* Enable roll over if FIFO over flows */
	max30102_maskRegister( driver_config, _register, ~_param, _param << shitf);
...
bool_t max30102_reset( max30102_t driver_config)
{
	...
	max30102_maskRegister( driver_config, MODE_CONFIG, ~MODE_RESET, MODE_RESET);
...
int16_t max30102_check()
{
	...
	if (readPointer != writePointer)
	{
		...
		if (_config.numberSamplesAvailable < 0)
			_config.numberSamplesAvailable += 32;
...
void max30102_maskRegister ( max30102_t driver_config, uint8_t _register, uint8_t _mask, uint8_t bitConfig )
{

	uint8_t registerActual = 0;

	driver_config._i2cReadFn(I2C0, MAX_ADDRESS, _register, &registerActual, 1);
	...
	driver_config._i2cWriteFn(I2C0, MAX_ADDRESS, _register, registerActual | bitConfig);
}
